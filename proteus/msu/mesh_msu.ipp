...
)
{
  ...
  int interior_bc = 0; // hardwire
  attrF.insert(interior_bc);
...
//if( bc == 0 ) // set interior faces for rans3p
//{
//  mesh.elementBoundaryMaterialTypes[index] = bc;
//  continue;
//}
...
)
{
  ...
  FOR( i, numF )
  {
    ...
    FOR(j,3) centroid[0] += x[vecN[j]-1]; // base0
    FOR(j,3) centroid[1] += y[vecN[j]-1]; // base0
    FOR(j,3) centroid[2] += z[vecN[j]-1]; // base0
